...
func TestABIEncoding(t *testing.T) {
	...
	if len(enc) != abiWitnessWords*32 {
		t.Fatalf("longueur ABI=%d, attendu %d", len(enc), abiWitnessWords*32)
	}
	if string(enc[0:32]) != string(pre[:]) {
		t.Fatal("preStateRoot")
	}
	if string(enc[32:64]) != string(post[:]) {
		t.Fatal("postStateRoot")
	}
	if binary.BigEndian.Uint32(enc[92:96]) != 0xaabbccdd {
		t.Fatalf("pc=%x", enc[92:96])
	}
	if enc[127] != 0x01 {
		t.Fatalf("opcode=%x", enc[127])
	}
	if binary.BigEndian.Uint64(enc[152:160]) != 0x0123456789abcdef {
		t.Fatalf("gas=%x", enc[152:160])
	}
	for i, want := range []uint64{1, 2, 3, 4} {
		got := evm256.FromBytesBE(enc[160+i*32 : 192+i*32])
		if got != evm256.FromU64(want) {
			t.Fatalf("stackIn[%d]=%v", i, got)
		}
	}
	for i, want := range []uint64{5, 6, 7, 8} {
		got := evm256.FromBytesBE(enc[288+i*32 : 320+i*32])
		if got != evm256.FromU64(want) {
			t.Fatalf("stackOut[%d]=%v", i, got)
		}
	}
	if binary.BigEndian.Uint32(enc[444:448]) != 0x100 {
		t.Fatalf("memOffset=%x", enc[444:448])
	}
	if string(enc[448:480]) != string(mem[:]) {
		t.Fatal("memData")
	}
	if evm256.FromBytesBE(enc[480:512]) != evm256.FromU64(9) {
		t.Fatal("storageKey")
	}
	if evm256.FromBytesBE(enc[512:544]) != evm256.FromU64(10) {
		t.Fatal("storageVal")
	}
}
